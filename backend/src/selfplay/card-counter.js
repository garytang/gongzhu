...
// Ties broken by card string so a choice never depends on the order of the hand.
const lowest = cards => cards.slice().sort(byRankAsc)[0];
const highest = cards => cards.slice().sort((a, b) => byRankAsc(b, a))[0];

/** Who is winning the trick as it stands. */
function currentBest(trick) {
  const ledSuit = suitOf(trick[0].card);
  return trick.reduce(
    (best, play) => (suitOf(play.card) === ledSuit && rankValue(play.card) > rankValue(best.card) ? play : best),
    trick[0],
  );
}
...
function playersAfterMe(obs) {
  const seat = obs.playerIds.indexOf(obs.playerId);
  const remaining = obs.playerIds.length - obs.trick.length - 1;
  const after = [];
  for (let i = 1; i <= remaining; i++) {
    after.push(obs.playerIds[(seat + i) % obs.playerIds.length]);
  }
  return after;
}

/** True when nobody left to act can outrank `best` in the led suit. */
function leadWillHold(obs, tracker, best, ledSuit, after) {
  const highest = tracker.highestOutstanding(ledSuit, obs.hand);
  if (!highest || rankValue(highest) <= rankValue(best.card)) return true;
  return after.every(id => tracker.isVoid(id, ledSuit));
}

/** True when someone still to act is void in the led suit and could dump on me. */
function dumpRisk(obs, tracker, ledSuit, after) {
  if (after.length === 0) return false;
  const dangerLeft = !tracker.played.has(PIG) || tracker.outstanding(HEARTS, obs.hand).length > 0;
  return dangerLeft && after.some(id => tracker.isVoid(id, ledSuit));
}
...
function harmlessDiscard(moves) {
  ...
  if (plain.length > 0) return highest(plain);
  return lowest(moves);
}
...
function dumpOnOpponent(moves, variant) {
  ...
  const hearts = moves.filter(isHeart).filter(c => cardValue(c, variant) < 0);
  if (hearts.length > 0) return highest(hearts);
  // Nothing painful to shed: drop the highest card that cannot win me a trick later,
...
function chooseLead(obs, tracker, opponents) {
  const moves = obs.legalMoves;
  const hand = obs.hand;
  const sheepOut = !tracker.played.has(SHEEP) && !hand.includes(SHEEP);
  const pigOut = !tracker.played.has(PIG) && !hand.includes(PIG);
  const heartsOut = tracker.outstanding(HEARTS, hand).length > 0;
  const holdPig = hand.includes(PIG);

  const scored = moves.map((card) => {
    const suit = suitOf(card);
    const rank = rankValue(card);
    const highest = tracker.highestOutstanding(suit, hand);
    const beatable = highest !== null && rankValue(highest) > rank;
    let score = -rank * 3;

    if (card === PIG) score -= 400;
    else if (card === SHEEP) score += beatable ? -250 : 300;
    ...
    if (beatable) score += 15;

    // Leading into a void hands a free discard to whoever is short.
    const voidOpponents = tracker.countVoid(opponents, suit);
    if (voidOpponents > 0 && (pigOut || heartsOut)) score -= 30 * voidOpponents;
    ...
    if (suit === DIAMONDS && sheepOut && !beatable) score += 45;

    return { card, score };
  });

  return scored.sort((a, b) => b.score - a.score || byRankAsc(a.card, b.card))[0].card;
}

function chooseDiscard(obs, tracker, teammate, ledSuit) {
  const moves = obs.legalMoves;
  const best = currentBest(obs.trick);
  const after = playersAfterMe(obs);
  const teammateTakesIt = teammate && best.player === teammate
    && leadWillHold(obs, tracker, best, ledSuit, after)
    && !after.includes(teammate);
...
function chooseFollow(obs, tracker, teammate, ledSuit) {
  ...
  const teammateWinning = teammate && best.player === teammate;
  const safeToWin = !dumpRisk(obs, tracker, ledSuit, after);
  const iWouldHold = winning.length > 0
    && leadWillHold(obs, tracker, { card: highest(winning) }, ledSuit, after);

  // Let a teammate keep a trick they are already going to take.
  if (teammateWinning && leadWillHold(obs, tracker, best, ledSuit, after) && ducking.length > 0) {
    return highest(ducking);
  }

  // Worth taking: the trick pays, or it is free and the lead is worth having.
  const worthTaking = iWouldHold && (value > 0 || (value === 0 && safeToWin && after.length === 0));
  if (worthTaking) {
    ...
    if (transformer && value > 0) return transformer;
    const withoutTransformer = winning.filter(c => c !== TRANSFORMER);
    const pool = withoutTransformer.length > 0 ? withoutTransformer : winning;
    return lowest(pool);
  }
...
function decide(obs, tracker) {
  ...
  const teammate = obs.teammate || null;
  const opponents = obs.playerIds.filter(id => id !== obs.playerId && id !== teammate);

  if (obs.trick.length === 0) return chooseLead(obs, tracker, opponents);

  const ledSuit = suitOf(obs.trick[0].card);
  if (moves.some(c => suitOf(c) === ledSuit)) return chooseFollow(obs, tracker, teammate, ledSuit);
  return chooseDiscard(obs, tracker, teammate, ledSuit);
}
...
module.exports = { cardCounterPolicy, decide };
